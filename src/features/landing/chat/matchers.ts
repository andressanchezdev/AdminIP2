...
function applyModifiers(
  ...
  if (ctx.lastTopIntent === intent || ctx.conversationFocus?.intent === intent) score += 1
  const recent = ctx.lastResponses.slice(-2).some((item) => item.intent === intent)
  if (recent && ctx.conversationFocus?.intent !== intent) score -= 1
    ...
    .map((intent) =>
      runSafe(intent.id, () => {
        ...
        if (
          ...
          wantsAdvisorContact(tokens) &&
          !tokens.some((token) =>
            ['direccion', 'ubicacion', 'ubicados', 'sede', 'local', 'sucursal', 'mapa', 'maps', 'horario', 'horarios', 'hora', 'horas', 'abre', 'abren', 'abierto', 'cierra', 'cierran', 'atencion', 'medellin', 'ciudad', 'encuentran', 'alpujarra', 'llegar'].includes(token),
          )
        ) {
          return null
        }
