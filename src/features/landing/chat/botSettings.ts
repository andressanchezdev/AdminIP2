...
import { LANDING_CONTACT, LANDING_PAYMENTS } from '../content'
...
export const DEFAULT_REPLIES: Record<string, BotReplyConfig> = {
  ...
  fallbackShort: {
    ...
    text: '"{term}" es muy corto para poder proporcionarte informacion de valor. Escribe el nombre de la pieza y el modelo. ',
    texts: [
      ...
      '"{term}" no me da un dato util. Dime la pieza y el vehiculo.',
    ],
  },
  fallbackLong: {
    keywords: 'palabra, termino, texto',
    text: '"{term}" no logro entender tu consulta. repite la consulta proporcionando mas informacion, dime la marca o el modelo.',
    texts: [
      '"{term}" no la ubique como pieza. Escribela completa o dime marca y modelo.',
      'No relacione "{term}". Si es una pieza, usa el nombre de catalogo.',
      '"{term}" es larga y no la asocie. Dime la pieza con marca o modelo.',
    ],
  },
  fallbackWide: {
    ...
    texts: [
      'Hay demasiada informacion junta. Resume: que pieza buscas y el modelo.',
      'Acorta el mensaje a una frase con la pieza y el modelo del vehiculo.',
      'No relacione el texto largo. Deja pieza + modelo, por ejemplo pastillas AKT 125.',
    ],
  },
  fallbackMixed: {
    keywords: 'mayusculas, minusculas, mezclado',
    text: 'No entendí "{term}" ya ue integra mayusculas y minusculas. Escríbelo normal y agrega la pieza o el producto.',
    texts: [
      '"{term}" mezcla mayusculas. Escribelo normal y agrega la pieza o el modelo.',
      'No lei bien "{term}". Usa mayusculas normales y dime la pieza.',
      'Escribe "{term}" sin mezclar mayusculas y suma la pieza o el producto.',
    ],
  },
  teamMember: {
  ...
  ackRepeat: {
    ...
    text: '¿Te refieres a lo mismo de antes?',
    texts: [
      'Veo que insistes. ¿Sigue siendo sobre {topic}?',
      'Como te comentaba, {topic}. ¿Quieres que avancemos en eso?',
    ],
    textsEn: [
