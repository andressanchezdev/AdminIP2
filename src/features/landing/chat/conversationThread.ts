...
export function nextConversationFocus(intent: string, ctx: SessionContext, score: number, minScore: number) {
  if (score < minScore) return ctx.conversationFocus
  if (keepConversationFocus(intent) && ctx.conversationFocus) return ctx.conversationFocus
  ...
  if (ctx.conversationFocus?.family && family && ctx.conversationFocus.family !== family && weak) {
    return { intent, label: rawLabel || intent, family: resolved?.id || partStem(rawLabel || intent), turn: ctx.turn }
  }
  return { intent, label: resolved?.label || label, family, turn: ctx.turn }
}
