...
export function attachSuggestions(reply: ChatReply, ctx: SessionContext): ChatReply {
  return { ...reply, actions: uniqueActions([...reply.actions, ...suggestionActions(ctx)]) }
}
...
export function combineCandidates(primary: Candidate, secundarios: Candidate[], tokens: readonly string[], ctx: SessionContext) {
  const absorbed: Record<string, string[]> = {
    catalog: ['whatsapp', 'attention', 'parts'],
    quote: ['whatsapp', 'attention', 'catalog', 'parts', 'product'],
    parts: ['catalog', 'whatsapp', 'attention'],
    product: ['catalog', 'quote', 'whatsapp', 'attention'],
    namedPart: ['parts', 'whatsapp'],
    location: ['whatsapp', 'attention'],
    credit: ['whatsapp', 'attention', 'payment'],
    payment: ['whatsapp', 'attention', 'catalog', 'product', 'quote'],
    shipping: ['whatsapp', 'attention', 'catalog'],
    orderStatus: ['whatsapp', 'attention', 'catalog', 'product', 'quote', 'shipping', 'location'],
    vacancy: ['whatsapp', 'attention', 'location'],
    company: ['whatsapp', 'attention'],
    complaint: ['whatsapp', 'attention'],
    teamGroup: ['attention', 'whatsapp', 'company', 'location'],
    teamMember: ['attention', 'whatsapp', 'company'],
    teamSuggest: ['attention', 'company'],
    attention: ['teamGroup', 'teamMember', 'teamSuggest', 'whatsapp', 'location'],
  }
  const explicitWhatsapp = (item: Candidate) =>
    item.matchedTokens.some((token) => ['whatsapp', 'telefono', 'correo', 'email', 'wsp', 'wa', 'celular'].includes(token))
  const skip = new Set(absorbed[primary.intent] ?? [])
  const extra = secundarios.filter((item) => {
    if (item.intent === primary.intent) return false
    if (skip.has(item.intent) && !(item.intent === 'whatsapp' && explicitWhatsapp(item))) return false
    return true
  })
  const ordered = [primary, ...extra].slice(0, 3)
