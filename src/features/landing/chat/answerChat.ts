...
import { countOccurrences, isDefineQuestion, isHowAreYou, isSmallTalk, isStopToken, preProcess, prioritizeLong, tokenizeRaw, tokensEqual, type PreparedInput } from './prepare'
...
function recoverUnmatched(ctx: SessionContext, raw: string, tokens: readonly string[], cfg: ReturnType<typeof configOf>): ChatReply {
  ...
  if (ctx.fallbackCount >= cfg.FALLBACK_MENU_AT) {
    ctx.offerMenu = true
    ctx.offerHumanHandoff = ctx.errorCount >= cfg.HUMAN_HANDOFF_AT
  ...
  if (tokens.some((token) => token.length > 2)) {
    return finalize(ctx, rectifyNeedPartReply(), 'fallback', 0, cfg)
  }
  return finalize(ctx, fallbackReply(raw), 'fallback', 0, cfg)
}
...
function finalize(
  ...
  if (score >= cfg.UMBRAL_MINIMO) {
    ...
    ctx.conversationFocus = nextConversationFocus(intent, ctx, score, cfg.UMBRAL_MINIMO)
    ctx.fallbackCount = 0
  }
  ...
  ctx.holdFocus = false
  next = { ...next, text: interpolate(next.text) }
  if (
